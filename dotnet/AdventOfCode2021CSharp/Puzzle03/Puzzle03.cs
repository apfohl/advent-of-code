...
namespace AdventOfCode2021CSharp.Puzzle03;
...
public static class Puzzle03
{
    ...
    private static int[] Calculate(this int[][] inputs,
    ...
    private static Func<IEnumerable<int[]>, int, IEnumerable<int[]>> Filter(Func<int, int, int> compare) =>
        (numbers, position) =>
        {
            var (zeros, ones, values) = numbers.Aggregate(new State(0, 0, []), (state, digits) =>
