...
namespace AdventOfCode2021CSharp.Puzzle04;
...
public static class Puzzle04
{
    ...
    private sealed class Field(int number)
    {
    ...
    private sealed class Board(Field[][] fields)
    {
    ...
    private sealed record Input(IEnumerable<int> Draws, IEnumerable<Board> Boards)
    {
        public static Input FromFile(string path)
        {
            ...
            var draws = reader.ReadLine()!.Split(',').Select(int.Parse);
            List<Board> boards = [];

            reader.ReadLine();

            List<Field[]> tempBoard = [];
    ...
    public static void PartTwo()
    {
        ...
        List<(Board board, int draw)> winners = [];
        foreach (var draw in input.Draws)
        {
            foreach (var board in input.Boards)
            {
